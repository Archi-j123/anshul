...
for line in log_data:
    ...
    last_token_match = re.search(r"last_token_recharge_time:\s*datetime\.datetime\(([^)]+)\)", line)
    data_type_match = re.search(r'data_type:\s*([A-Za-z0-9]+)', line)
    ...
    # Process the last_token_recharge_time field if matched
    if last_token_match:
        last_token_date_str = last_token_match.group(1)
        # Parse the extracted datetime components
        try:
            last_token_date_obj = datetime.strptime(last_token_date_str, "%Y, %m, %d, %H, %M")
            # Separate date and time into different formats
            row_data['last_token_recharge_date'] = last_token_date_obj.strftime("%d-%m-%Y")
            row_data['last_token_recharge_time'] = last_token_date_obj.strftime("%I:%M:%S")
        except ValueError:
            row_data['last_token_recharge_date'] = None
            row_data['last_token_recharge_time'] = None

    # Add data_type if matched
    if data_type_match:
        row_data['data_type'] = data_type_match.group(1)
