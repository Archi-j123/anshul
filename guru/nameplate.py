...
exec(open('C:\\Users\\krishankanty\\Desktop\\anshul\\data_preparation\\nameplate.py').read()) 
...
output_file_path = os.path.join(os.getenv('USERPROFILE'), 'Downloads', 'nameplate.xlsx')
...
# Loop through each row to parse data based on your structure
for row in rows:
    # Assuming data is space-separated, split the row into values
    values = row.split()
    
    # Map the values to the correct fields based on position
    if len(values) >= 9:  # Check to ensure there are enough values in the row
        meter_no = values[0]
        SM_device_id = values[1]
        manufacturer_name = " ".join(values[2:7])  # Manufacturer name may span multiple words
        firmware_version = values[7]
        type_ = values[8]
        category = values[9]
        current_rating = values[10] if len(values) > 10 else None  # Optional field
        I_symbol = values[11] if len(values) > 11 else None  # Optional field
        manufacturing_year = values[12] if len(values) > 12 else None  # Optional field

        # Append the parsed row as a list of values
        parsed_data.append([meter_no, SM_device_id, manufacturer_name, firmware_version, 
                            type_, category, current_rating, I_symbol, manufacturing_year])

# Define headers for data_guruX.txt
headers_file1 = ['meter_no', 'SM_device_id', 'manufacturer_name', 'firmware_version', 'type', 
                'category', 'current_rating', 'I Symbol', 'manufacturing_year']
...
for col in ['firmware_version' , 'type','manufacturing_year']:
...
columns_file2 = ['SM_device_id','manufacturer_name', 'firmware_version' , 'type','category', 'current_rating','manufacturing_year']
...
comparison_columns = ['SM_device_id','manufacturer_name', 'firmware_version' , 'type','category', 'current_rating','manufacturing_year']
for col in comparison_columns:
    result_column = f"{col}_result"
    df_merged[result_column] = df_merged.apply(
        lambda row, col=col: 'PASS' if row[f'file1_{col}'] == row[f'file2_{col}'] else 'FAIL',
        axis=1
    )
